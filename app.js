...
global.__middlewares_dir = __basedir + '/middlewares';
...
global.__random = require(__class_dir + '/hash.class.js').randomString(
  40,
  'base64'
);
...
function getRoutersSync(__path, __extension) {
  ...
  fs.readdirSync(__path).forEach((file) => {
    ...
    if (stats.isFile() && path.extname(file) === __extension) {
      files[path.basename(file, path.extname(file))] = path.resolve(
        __path,
        file
      );
...
for (const mainRoute in routers) {
  for (const subRoute in routers[mainRoute]) {
    app.use(
      `${mainRoute === '/' ? '' : mainRoute}/${subRoute}`,
      require(routers[mainRoute][subRoute])
    );
  }
}
